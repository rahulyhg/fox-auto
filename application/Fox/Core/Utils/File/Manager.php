...
class Manager
{
    ...
    {
        $fullPath = __ROOT__ . '/' . $this->concatPaths($path);
    ...
    public function unsetContents($path, $unsets, $isJSON = true)
    {
        ...
        if ($currentData == false) {
            logger()->notice('FileManager::unsetContents: File ['.$this->concatPaths($path).'] does not exist.');
            return false;
        }
    ...
    public function mkdir($path, $permission = null, $recursive = false)
    {
        ...
        try {
            ...
            logger()->critical('Permission denied: unable to create the folder on the server - '.$fullPath);
        }
...
//         return false;
    ...
    /**
     * Get exists path. Ex. if check /var/www/CRM/custom/someFile.php and this file doesn't extist, result will be /var/www/CRM/custom
     *
     * @param  string | array $path
     *
     * @return string
