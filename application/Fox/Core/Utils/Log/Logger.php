<?php
namespace Fox\Core\Utils\Log;

use \Fox\Core\Utils\Log\Base;
use \Fox\Core\Support\Arr;
use \Fox\Core\Utils\Log\Logger as Monolog;
use \Fox\Core\Utils\Log\Handler\DebugHandler;
...
class Logger extends Base 
{
	...
	public function db($code, $msg, $line, $file) 
	{
		...
		$msg .= ' [' . REQUEST_METHOD . ': ' . __URI__ . ', ' . $file . '(' . $line . ')]';
		
		$channel->addError($msg);
	}
	...
	protected function pushHandlers(Monolog $channel, array $config) 
	{
		...
		foreach ($handlers as & $info) {//添加日志处理器
			...
			$handelClass = '\\Fox\Core\\Utils\\Log\\Handler\\' . $handler;
			...
			if (! empty($info['formatter'])) {
				$fomatterClass = '\\Fox\Core\\Utils\\Log\\Formatter\\' . $info['formatter'];
	...
	public function debug($msg, $path = null) 
	{
		if(! isset($this->instances['debug'])) {
			$this->instances['debug'] = new Monolog('debug');
			if (! $path) {
				$path = 'data/logs/debug.log';
			}
			
			$this->instances['debug']->pushHandler(new DebugHandler($path));
		}
