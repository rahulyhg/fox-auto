<?php
namespace Fox\Core\Utils\Metadata;
...
class Helper
{
    ...
    public function __construct(\Fox\Core\Utils\Metadata $metadata)
    {
    ...
    public function getFieldDefsByType($fieldDef)
    {
        ...
        if (isset($fieldDef['dbType'])) {
            return $this->metadata->get('fields.'.$fieldDef['dbType']);
    ...
    public function getAdditionalFieldList($fieldName, array $fieldParams, array $definitionList = null)
    {
        ...
        $fieldDefinition = isset($definitionList[$fieldType]) ? $definitionList[$fieldType] : $this->metadata->get('fields.'.$fieldType);
