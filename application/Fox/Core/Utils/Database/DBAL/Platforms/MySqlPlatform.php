...
namespace Fox\Core\Utils\Database\DBAL\Platforms;
...
class MySqlPlatform extends \Doctrine\DBAL\Platforms\MySqlPlatform
{
    public function getAlterTableSQL(TableDiff $diff)
    {
        ...
        //fox: It works not correctly. It can rename some existing fields
        foreach ($diff->renamedColumns as $oldColumnName => $column) {
            if ($this->onSchemaAlterTableRenameColumn($oldColumnName, $column, $diff, $columnSql)) {
                continue;
            }

            //fox: remaned autoincrement field
            ...
            $queryParts[] = 'ADD ' . $this->getColumnDeclarationSQL($column->getQuotedName($this), $columnArray); //fox: fixed the problem
        } //fox: END
        ...
        foreach ($diff->removedColumns as $column) {
            ...
            //fox: remove autoincrement option
            if ($column->getAutoincrement()) {
            ...
            //$queryParts[] =  'DROP ' . $column->getQuotedName($this); //fox: no needs to remove columns
        }
    ...
    //fox: fix problem with quoting table name
