...
class Schema
{
    ...
    protected $drivers = array(
        'mysqli' => '\Fox\Core\Utils\Database\DBAL\Driver\Mysqli\Driver',
        'pdo_mysql' => '\Fox\Core\Utils\Database\DBAL\Driver\PDOMySql\Driver',
    );

    protected $fieldTypePaths = array(
        'application/Fox/Core/Utils/Database/DBAL/FieldTypes',
        'custom/Fox/Custom/Core/Utils/Database/DBAL/FieldTypes',
    );
    ...
    protected $rebuildActionsPath = array(
        'corePath' => 'application/Fox/Core/Utils/Database/Schema/rebuildActions',
        'customPath' => 'custom/Fox/Custom/Core/Utils/Database/Schema/rebuildActions',
    );
    ...
    public function __construct(\Fox\Core\Utils\Config $config, \Fox\Core\Utils\Metadata $metadata, \Fox\Core\Utils\File\Manager $fileManager, \Fox\Core\ORM\EntityManager $entityManager, \Fox\Core\Utils\File\ClassParser $classParser)
    {
        $this->config = $config;
        ...
        $this->comparator = new \Fox\Core\Utils\Database\DBAL\Schema\Comparator();
        $this->initFieldTypes();

        $this->converter = new \Fox\Core\Utils\Database\Converter($this->metadata, $this->fileManager);
    }
...
debug($queries);die;
        ...
        foreach ($queries as $sql) {
            logger()->info('SCHEMA, Execute Query: '.$sql);
            try {
                $result &= (bool) $connection->executeQuery($sql);
            } catch (\Exception $e) {
                logger()->alert('Rebuild database fault: '.$e);
                $result = false;
            }
        }
