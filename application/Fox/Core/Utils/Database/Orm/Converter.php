<?php
namespace Fox\Core\Utils\Database\Orm;

use Fox\Core\Utils\Util,
    Fox\ORM\Entity;
use \Fox\Core\Utils\Metadata\Helper;
...
class Converter
{
    ...
    */
    ...
    public function __construct(\Fox\Core\Utils\Metadata $metadata, \Fox\Core\Utils\File\Manager $fileManager, Helper $helper)
    {
    ...
    public function process()
    {
        ...
        foreach ($entityDefs as $entityName => & $entityMeta) {
            if (empty($entityMeta)) {
                logger()->critical('Orm\Converter:process(), Entity:'.$entityName.' - metadata cannot be converted into ORM format');
                continue;
            }
    ...
    /**
     * Metadata conversion from Fox format into Doctrine
     *
    ...
    protected function convertFields($entityName, &$entityMeta)
    {
        ...
        foreach($entityMeta['fields'] as $fieldName => & $fieldParams) {
            ...
            $fieldTypeMeta = $this->metadataHelper->getFieldDefsByType($fieldParams);
            ...
            if (isset($fieldTypeMeta['linkDefs'])) {
                $linkDefs = $this->metadataHelper->getLinkDefsInFieldMeta($entityName, $fieldParams, $fieldTypeMeta['linkDefs']);
                if (isset($linkDefs)) {
    ...
    /**
     * Correct fields defenitions based on \Fox\Custom\Core\Utils\Database\Orm\Fields
     *
     * @param  array  $ormMeta
     *
     * @return array
    ...
    protected function correctFields($entityName, array $ormMeta)
    {
        ...
        foreach ($entityMeta['fields'] as $fieldName => $fieldParams) {
            ...
            $className = '\Fox\Custom\Core\Utils\Database\Orm\Fields\\' . $fieldType;
            if (!class_exists($className)) {
                $className = '\Fox\Core\Utils\Database\Orm\Fields\\' . $fieldType;
            }
        ...
        $scopeDefs = $this->metadata->get('scopes.'.$entityName);
    ...
    protected function convertLinks($entityName, $entityMeta, $ormMeta)
    {
        ...
        foreach ($entityMeta['links'] as $linkName => $linkParams) {
            ...
            $convertedLink = $this->relationManager->convert($linkName, $linkParams, $entityName, $ormMeta);
