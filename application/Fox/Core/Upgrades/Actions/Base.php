...
abstract class Base
{
    ...
    public function __construct(\Fox\Core\Container $container, \Fox\Core\Upgrades\ActionManager $actionManager)
    {
        $this->container = $container;
        $this->actionManager = $actionManager;
        $this->params = $actionManager->getParams();

        $this->zipUtil = new \Fox\Core\Utils\File\ZipArchive($container->make('fileManager'));
    }
    ...
    protected function getFileManager()
    {
        if (!isset($this->fileManager)) {
            $this->fileManager = $this->getcontainer()->make('fileManager');
        }
    ...
    public function getEntityManager()
    {
        if (!isset($this->entityManager)) {
            $this->entityManager = $this->getcontainer()->make('entityManager');
        }
    ...
    public function checkVersions($versionList, $currentVersion, $errorMessage = '')
    {
        ...
        try {
            ...
            logger()->error('Cannot recognize currentVersion ['.$currentVersion.'], error: '.$e->getMessage().'.');
            return false;
        }
        ...
        foreach ($versionList as $version) {
            ...
            try {
                ...
                logger()->error('Version identification error: '.$e->getMessage().'.');
            }
    ...
    {
        return $this->getcontainer()->make('dataManager')->rebuild();
    }
    ...
    {
        return $this->getcontainer()->make('dataManager')->clearCache();
    }
