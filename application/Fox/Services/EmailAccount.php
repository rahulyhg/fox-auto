    ...
    public function fetchFromMailServer(Entity $emailAccount)
    {
        ...
        foreach ($monitoredFoldersArr as $folder) {
            ...
            try {
                ...
                logger()->error('EmailAccount '.$emailAccount->id.' (Select Folder) [' . $e->getCode() . '] ' .$e->getMessage());
                continue;
            }
            ...
            foreach ($ids as $i => $id) {
                ...
                try {
                    ...
                    try {
    	                ...
    	                logger()->error('EmailAccount '.$emailAccount->id.' (Import Message): [' . $e->getCode() . '] ' .$e->getMessage());
    	            }
                    ...
                    logger()->error('EmailAccount '.$emailAccount->id.' (Get Message): [' . $e->getCode() . '] ' .$e->getMessage());
                }
