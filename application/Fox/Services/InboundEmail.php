...
use \Fox\ORM\Entity;
use \Fox\Entities\Team;
use \Fox\Entities\Email;

use \Fox\Core\Exceptions\Error;
use \Fox\Core\Exceptions\Forbidden;

class InboundEmail extends \Fox\Services\Record
{
    ...
    public function fetchFromMailServer(Entity $emailAccount)
    {
        ...
        foreach ($monitoredFoldersArr as $folder) {
            ...
            try {
                ...
                logger()->error('InboundEmail '.$emailAccount->id.' (Select Folder) [' . $e->getCode() . '] ' .$e->getMessage());
                continue;
            }
                    ...
                    if ($message && isset($message->from)) {
                        ...
                        if (preg_match('/MAILER-DAEMON|POSTMASTER/i', $fromString)) {
                            ...
                            try {
                                ...
                                logger()->error('InboundEmail '.$emailAccount->id.' (Process Bounced Message: [' . $e->getCode() . '] ' .$e->getMessage());
                            }
                        }
                    }
                    if (!$toSkip) {
                        try {
                            ...
                            logger()->error('InboundEmail '.$emailAccount->id.' (Import Message): [' . $e->getCode() . '] ' .$e->getMessage());
                        }
                    }
                } catch (\Exception $e) {
                    logger()->error('InboundEmail '.$emailAccount->id.' (Get Message): [' . $e->getCode() . '] ' .$e->getMessage());
                }
    ...
    {
        $className = '\\Fox\\Custom\\Business\\Distribution\\CaseObj\\RoundRobin';
        if (!class_exists($className)) {
            $className = '\\Fox\\Modules\\Crm\\Business\\Distribution\\CaseObj\\RoundRobin';
        }
    ...
    {
        $className = '\\Fox\\Custom\\Business\\Distribution\\CaseObj\\LeastBusy';
        if (!class_exists($className)) {
            $className = '\\Fox\\Modules\\Crm\\Business\\Distribution\\CaseObj\\LeastBusy';
        }
    ...
    protected function emailToCase(\Fox\Entities\Email $email, array $params = array())
    {
